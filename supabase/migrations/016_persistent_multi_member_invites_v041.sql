-- v0.41 - persistent 24-hour multi-member invite codes for Noi ♡.
-- Invite plaintext is never exposed through the Data API: authenticated has no
-- table privileges and the existing deny-select RLS policy remains in place.

alter table public.space_invites
  add column if not exists code_value text,
  add column if not exists revoked_at timestamptz;

-- Legacy invites cannot reveal their original plaintext code, so invalidate
-- only still-active legacy rows once. New v0.41 invites remain reusable until
-- expiry or explicit regeneration.
...
-- Backwards compatibility for pre-v0.41 clients: reopening the invite no
-- longer rotates it.
...
begin
  ...
  -- v0.41 invite codes are intentionally multi-use for their full 24-hour TTL.
  -- consumed_at is retained only for historical compatibility.
